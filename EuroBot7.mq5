...
input string          VER                  = "E6";   // Prefixo da mensagem Telegram


input group " "
input group "PROVISORIOS"

input ENUM_TIMEFRAMES OutroTF     = PERIOD_M30;
input int             BB_Period   = 20;
input double          BB_Dev      = 2.0;
input ENUM_APPLIED_PRICE BB_Price = PRICE_CLOSE;

input color CorUpper  = clrRed;
input color CorMiddle = clrOrange;
input color CorLower  = clrRed;

input int    LarguraLinhas   = 2;
input int    SubSegmentos    = 8;      // Quantos segmentos por transição (4-12 é bom equilíbrio)
input double PorcentagemBarra = 0.98;  // Quase toda a barra

string Prefixo = "MTFBB_Smooth_";
...
input
double             Fator_Reabre_OP_em_Gain      = 1.5;     // Pontos para reentrada gain
input
double             width      = 0;     // Largura linha Bollinger (pontos)
input
int             Notick      = 60;     // Tempo para rever ops no tick (Segs)
input
double  Sdo_THRESHOLD = 200;// Max QUEDA SALDO inicial
input
double  Ops_THRESHOLD = 200;// Max QUEDA SALDO operacional
...
int OnInit()
  {
   ...
   INV_oper = inverso;
   handleBB = iBands(_Symbol, OutroTF, BB_Period, 0, BB_Dev, BB_Price);
   if(handleBB == INVALID_HANDLE)
     {
      Print("Erro iBands: ", GetLastError());
      return(INIT_FAILED);
     }

   DeletarTodosObjetos();
   Print("Iniciado - Suavização com ", SubSegmentos, " sub-segmentos");
...
void AtualizarBandasSuavizadas()
{
   int limit = MathMin(150, Bars(_Symbol, PERIOD_CURRENT));
   ...
   int copied_upper  = CopyBuffer(handleBB, 1, 0, limit, upper);
   int copied_middle = CopyBuffer(handleBB, 0, 0, limit, middle);
   int copied_lower  = CopyBuffer(handleBB, 2, 0, limit, lower);
   int copied_time   = CopyTime(_Symbol, PERIOD_CURRENT, 0, limit, tempo);
   
   if(copied_upper <= 0 || copied_middle <= 0 || copied_lower <= 0 || copied_time <= 0)
   {
      Print("ERRO Copy: upper=", copied_upper, " middle=", copied_middle, 
            " lower=", copied_lower, " time=", copied_time, " | Erro=", GetLastError());
      return;
   }
   
   Print("Copiou ", copied_upper, " barras OK. Exemplo: upper[0]=", DoubleToString(upper[0], _Digits),
         " | tempo[0]=", TimeToString(tempo[0]), " (mais recente)");
   
   DeletarTodosObjetos();
   
   int obj_count = 0;
   int transicoes_validas = 0;
   
   // Loop do mais antigo (índice alto) para o mais novo (índice baixo)
   for(int i = limit - 2; i >= 0; i--)   // Começa em limit-2, vai até 0
   {
      // Ponto atual (mais novo no loop): i
      // Próximo ponto (mais futuro): i-1 ? Não — como estamos indo para trás, "próximo" é i+1 no array (mais antigo)
      // Melhor: definir left (antigo) e right (novo)
      
      int idx_left  = i + 1;   // Mais antigo (maior índice no array série)
      int idx_right = i;       // Mais novo (menor índice)
      
      if(upper[idx_left] <= 0 || upper[idx_left] == EMPTY_VALUE ||
         upper[idx_right] <= 0 || upper[idx_right] == EMPTY_VALUE) continue;
      
      if(middle[idx_left] <= 0 || middle[idx_right] <= 0 ||
         lower[idx_left] <= 0 || lower[idx_right] <= 0) continue;
      
      datetime t_left  = tempo[idx_left];   // Tempo mais antigo
      datetime t_right = tempo[idx_right];  // Tempo mais novo
      
      long duracao = t_right - t_left;
      
      if(duracao <= 0)
      {
         Print("Ainda duração inválida na barra idx=", i, ": ", duracao,
               " | t_left=", TimeToString(t_left), " t_right=", TimeToString(t_right));
         continue;
      }
      
      transicoes_validas++;
      
      SuavizarLinha("U", i, t_left, upper[idx_left], t_right, upper[idx_right], duracao, obj_count);
      SuavizarLinha("M", i, t_left, middle[idx_left], t_right, middle[idx_right], duracao, obj_count);
      SuavizarLinha("L", i, t_left, lower[idx_left], t_right, lower[idx_right], duracao, obj_count);
   }
   
   ChartRedraw(0);
   Print("Atualizado - ", obj_count, " objetos criados | Transições válidas: ", transicoes_validas,
         " de ", limit-1, " possíveis");
}
//+------------------------------------------------------------------+
//| Cria sub-segmentos retos para simular curva suave                |
//+------------------------------------------------------------------+
void SuavizarLinha(string tipo, int idx_base, datetime t1, double p1, datetime t2, double p2, long duracao, int &contador)
  {
   double delta_p = p2 - p1;
   long delta_t   = duracao / SubSegmentos;

   for(int s = 0; s < SubSegmentos; s++)
     {
      datetime ts = t1 + s * delta_t;
      datetime te = ts + delta_t;
      if(s == SubSegmentos - 1)
         te = t2;  // Último vai até o fim exato

      double ps = p1 + (s * delta_p / SubSegmentos);
      double pe = p1 + ((s+1) * delta_p / SubSegmentos);

      string nome = Prefixo + tipo + "_" + IntegerToString(idx_base) + "_" + IntegerToString(s);

      if(ObjectCreate(ChartID(), nome, OBJ_TREND, 0, ts, ps, te, pe))
        {
         ObjectSetInteger(ChartID(), nome, OBJPROP_COLOR, (tipo=="U") ? CorUpper : (tipo=="M") ? CorMiddle : CorLower);
         ObjectSetInteger(ChartID(), nome, OBJPROP_STYLE, STYLE_SOLID);
         ObjectSetInteger(ChartID(), nome, OBJPROP_WIDTH, LarguraLinhas);
         ObjectSetInteger(ChartID(), nome, OBJPROP_RAY, false);
...
//+------------------------------------------------------------------+
//| Limpeza total (simples, já que redesenha tudo)                   |
//+------------------------------------------------------------------+
void DeletarTodosObjetos()
  {
   for(int i = ObjectsTotal(ChartID(), 0, -1) - 1; i >= 0; i--)
     {
      string nome = ObjectName(ChartID(), i);
