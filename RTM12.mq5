...
//+------------------------------------------------------------------+
// 1.1 FATN LIMITADO A 1
// 1.6 Q DESP H2 M30
// 1.7 PRICE DIRECTION
// 1.8 SIMULADOR
// 1.9 TRAILING STOPS
// 2.0 NOVO ABRE OPER, MAX DIA GAIN
//+------------------------------------------------------------------+
sinput string   Robo = "RTM12 ";
sinput string Versão ="M12 2P";                  //
input bool PROP = false;
input int I_MainOP  =          0;             //  Método
input int I_MM_media_lenta  =          20;            //  Media gatilho, lenta
input int I_MM_media_media  =          10;            //  Media gatilho, intermediaria
input int I_MM_media_rapida  =          3;            //  Media gatilho, rápida
input int I_Max_dist  =          8;            //  Distancia max para previsão de cruzamento
input double I_MAX_LOTE  =          0.5;             //   Lote maximo para operar
input double I_SL_size  =           50;            //   Tamanho maximo do LOSS (USD)
input bool I_inverso  =          false;            //   Inverter as operações
input int I_dias  =          30;            //  Renova saldo
input int I_LimLAT  =          85;            //  Limite indicador lateralização
input double I_DIA_MAX_USD_GAIN  =          0;            //   Max USD DIA GAIN (Fecha dia)
input double I_DIA_MAX_USD_LOSS  =          0;            //   Max USD DIA LOSS (Fecha dia)
input int I_XPROP  =          0;            //  Proporcional ao saldo 0, 1, 2
input double I_CUR_MAX_USD_GAIN  =          0;            //   OP Max USD SALDO GAIN
input double I_CUR_MAX_USD_LOSS  =          0;            //   OP Max USD SALDO LOSS
input bool XTRS = false; // Trailing stops
input bool OPTX = true; //Conflitos
...
int dias;
int LimLAT;
double DIA_MAX_USD_GAIN;
double DIA_MAX_USD_LOSS;
int XPROP;
double CUR_MAX_USD_GAIN;
double CUR_MAX_USD_LOSS;
int W_OP[15];
int W_Q1=0;
int W_Q2=0;
...
void RESET_INPUTS()
  {
   MainOP = I_MainOP;
   MM_media_lenta = I_MM_media_lenta;
   MM_media_media = I_MM_media_media;
   MM_media_rapida = I_MM_media_rapida;
   Max_dist = I_Max_dist;
   MAX_LOTE = I_MAX_LOTE;
   SL_size = I_SL_size;
   inverso = I_inverso;
   dias = I_dias;
   LimLAT = I_LimLAT;
   DIA_MAX_USD_GAIN = I_DIA_MAX_USD_GAIN;
   DIA_MAX_USD_LOSS = I_DIA_MAX_USD_LOSS;
   XPROP = I_XPROP;
   CUR_MAX_USD_GAIN = I_CUR_MAX_USD_GAIN;
   CUR_MAX_USD_LOSS = I_CUR_MAX_USD_LOSS;
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void PROPORC_INPUTS()
  {
   double FATN = NormalizeDouble((SALDO_Corrigido() / STR_SDO_INI_ROBO),2);
   if(FATN > 1)
     {
      Print("FATN ",FATN);
     }
   ...
   MAX_LOTE = I_MAX_LOTE * FATN;
   SL_size = I_SL_size * FATN;
   DIA_MAX_USD_GAIN = I_DIA_MAX_USD_GAIN * FATN;
   DIA_MAX_USD_LOSS = I_DIA_MAX_USD_LOSS * FATN;
   CUR_MAX_USD_GAIN = I_CUR_MAX_USD_GAIN * FATN;
   CUR_MAX_USD_LOSS = I_CUR_MAX_USD_LOSS*  FATN;
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
//|                                                                  |
//+------------------------------------------------------------------+
int VerificarDirecaoEMA_Simples(int N, int Q)
  {
   int retop = 0;
   double ema_atual = AMedia(N, 1);
   double ema_anterior =AMedia(N, Q);
   if(ema_atual > 0.1 && ema_anterior > 0.1)
     {
      if(ema_atual > ema_anterior)
        {
         retop = 1;
        }
      if(ema_atual < ema_anterior)
...
//+------------------------------------------------------------------+
//| Retorna o valor da Média Móvel de N períodos, há Q barras atrás |
//| Ex: AMedia(20, 0)  → valor atual da MM20                        |
//|     AMedia(20, 5)  → valor da MM20 há 5 candles                 |
//+------------------------------------------------------------------+
...
// Se mudou o período ou o tipo da média → recria o handle
   if(handle == INVALID_HANDLE || ultimo_N != N || ultimo_metodo != metodo)
     {
      if(handle != INVALID_HANDLE)
         IndicatorRelease(handle);                    // libera o antigo
...
// Pega só o valor que você quer (1 valor apenas)
   double valor[1];
...
void CRITERIOS()
  {
   ...
   W_Q2=0;
   OPM=11;
   OPO = CROSS_MEDIAS(_Symbol, MM_media_rapida, MM_media_lenta);
   if(OPO > 0)
     {
      Save_W_OP(OPM, OPO);
     }
   OPM=12;
   OPO = GUESS(MM_media_rapida, MM_media_lenta);
   if(OPO > 0)
     {
      Save_W_OP(OPM, OPO);
     }
   OPM=13;
   OPO = CROSS_MEDIAS(_Symbol, MM_media_rapida, MM_media_media);
   if(OPO > 0)
     {
      Save_W_OP(OPM, OPO);
     }
   OPM=14;
   OPO = GUESS(MM_media_rapida, MM_media_media);
   if(OPO > 0)
     {
      Save_W_OP(OPM, OPO);
     }
   OPM=15;
   OPO = CROSS_MEDIAS(_Symbol, MM_media_media, MM_media_lenta);
   if(OPO > 0)
     {
      Save_W_OP(OPM, OPO);
     }
   OPM=16;
   OPO = GUESS(MM_media_media, MM_media_lenta);
   if(OPO > 0)
     {
      Save_W_OP(OPM, OPO);
     }
   OPM=21;
   OPO = PROX_DA_MEDIA(MM_media_rapida);
   if(OPO > 0)
     {
      Save_W_OP(OPM, OPO);
     }
   OPM=22;
   OPO = PROX_DA_MEDIA(MM_media_media);
   if(OPO > 0)
     {
      Save_W_OP(OPM, OPO);
     }
   OPM=23;
   OPO = PROX_DA_MEDIA(MM_media_lenta);
   if(OPO > 0)
     {
      Save_W_OP(OPM, OPO);
     }
   OPM=31;
   OPO = GAIN_AG(MM_media_rapida);
   if(OPO > 0)
     {
      Save_W_OP(OPM, OPO);
     }
   OPM=32;
   OPO = GAIN_AG(MM_media_media);
   if(OPO > 0)
     {
      Save_W_OP(OPM, OPO);
     }
   OPM=33;
   OPO = GAIN_AG(MM_media_lenta);
   if(OPO > 0)
     {
      Save_W_OP(OPM, OPO);
     }
   OPM=41;
   OPO = CheckBollingerSignal(Symbol(), PERIOD_CURRENT, 20, 2);
   if(OPO > 0)
     {
      Save_W_OP(OPM, OPO);
     }
   ...
   OPO = GetMeanReversionSignal(55, PERIOD_CURRENT, dist, ma_up, price_above);
   if(OPO > 0)
     {
      Save_W_OP(OPM, OPO);
     }
   ...
   V_CURRENT_BID = SymbolInfoDouble(Symbol(),SYMBOL_BID);
   SPREAD = V_CURRENT_ASK - V_CURRENT_BID;
   int DMP = VerificarDirecaoEMA_Simples(MM_media_rapida, Max_dist);
   int DMM = VerificarDirecaoEMA_Simples(MM_media_media, Max_dist);
   int DMG = VerificarDirecaoEMA_Simples(MM_media_lenta, Max_dist);
...
//|                                                                  |
//+------------------------------------------------------------------+
void Save_W_OP(int Criterio, int C_CV)
  {
   int Noper = MathAbs(Criterio/10);
   if(C_CV == 1)
     {
      W_Q1++;
     }
...
//   Print("OP ",Noper," ",Criterio," ",C_CV," ",W_Q1," ",W_Q2);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
