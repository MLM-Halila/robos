...
//+------------------------------------------------------------------+
// 1.4 highest and lowest
// 1.5 P para btcusd fim de semana
// 1.6 max_oper
// 1.7 Q OP para inverso
// 1.8 P REAL
// 1.9 MAINOP 6 VOLTA PARA A MEDIA
// 1.9 EMERG
// 2.0 SLSIZE & DMAXLOSS & CMAXLOSS
// 3.0 PROPORCIONAL AO SALDO
//+------------------------------------------------------------------+
sinput string   Robo = "RTM10";
//ATR LATERALIZ "RMV5GOLD - Para GOLD H2"
sinput string Versão ="M10 3.0";
//input double BASE_SDO = 0; //Saldo Base T

input bool PROP = false; //Proporcional ???
input int  MainOP                 = 6;                   //Metodo
input bool inverso             = false;               // Inverter as operações
input int MM_media_lenta = 20;         //Media gatilho, lenta
input int MM_media_media = 10;         //Media gatilho, intermediaria
input int MM_media_rapida = 3;         //Media gatilho, rapida
input int Max_dist = 8;         //Distancia max para previsão de cruzamento
input int dias = 20;         //Renova saldo
input int LimLAT = 85;         //Limite indicador lateralização
input int XPROP               = 1;         //Proporcional ao saldo 0, 1, 2

input double FT_MAX_LOTE = 0.0003; // Fator  FT_MAX_LOTE
input double FT_SL_size = 0.2; // Fator  FT_SL_size 
input double FT_DIA_GAIN = 0; // Fator  FT_DIA_GAIN 
input double FT_DIA_LOSS = 0; // Fator  FT_DIA_LOSS 
input double FT_CUR_GAIN = 0; // Fator  FT_CUR_GAIN 
input double FT_CUR_LOSS = 0; // Fator  FT_CUR_LOSS 





double I_MAX_LOTE = 2;                       // Lote maximo para operar
double I_SL_size    =  200;               // Tamanho maximo do LOSS (USD)
double I_DIA_MAX_USD_GAIN = 0;          // Max USD DIA GAIN (Fecha dia)
double I_DIA_MAX_USD_LOSS = 0;          // Max USD DIA LOSS (Fecha dia)
double I_CUR_MAX_USD_GAIN = 0;          // OP Max USD SALDO GAIN
double I_CUR_MAX_USD_LOSS = 0;          // OP Max USD SALDO LOSS
...
void RESET_INPUTS()
  {
   double SALDO_REF = STR_SDO_INI_ROBO; 
   MAX_LOTE = NormalizeDouble((SALDO_REF * FT_MAX_LOTE),2);
   SL_size = NormalizeDouble((SALDO_REF * FT_SL_size),2);
   DIA_MAX_USD_GAIN = NormalizeDouble((SL_size * FT_DIA_GAIN),2);
   DIA_MAX_USD_LOSS = NormalizeDouble((SL_size * FT_DIA_LOSS),2);
   CUR_MAX_USD_GAIN = NormalizeDouble((SL_size * FT_CUR_GAIN),2);
   CUR_MAX_USD_LOSS = NormalizeDouble((SL_size * FT_CUR_LOSS),2);
...
void PROPORC_INPUTS()
  {
   double SALDO_REF = SALDO_Corrigido(); 
   MAX_LOTE = NormalizeDouble((SALDO_REF * FT_MAX_LOTE),2);
   SL_size = NormalizeDouble((SALDO_REF * FT_SL_size),2);
   DIA_MAX_USD_GAIN = NormalizeDouble((SL_size * FT_DIA_GAIN),2);
   DIA_MAX_USD_LOSS = NormalizeDouble((SL_size * FT_DIA_LOSS),2);
