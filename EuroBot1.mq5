...
sinput string   Robo = "EuroBot1";
sinput string Versao ="2.26";                  //
input group " "
...
input double          LOTE               = 0.07;   // Lote base (inicial)
input group " "
...
input
double FATOR                      = 2.0;   // Fator prox oper
input
double             Fator_Reabre_OP_em_Gain      = 0.6;     // Pontos para reentrada gain
input
...
//   Print ("X saldo init ",s);
   ...
   CLote = NormalizeDouble((PropLote * s / 100),2);
//Print("ERRO "," LOTE ",LOTE," s ",s," CLote ",CLote," CCCC ",CCCC);
...
// Copia os valores das bandas
   ...
   */
   if(price_prev > upper[1])
      return 2;

   if(price_prev < lower[1])
      return 1;
   return 0;
  }
//+------------------------------------------------------------------+
