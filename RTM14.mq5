...
MAIN OP
  ...
  2 || 0 PROX_DA_MEDIA
  3 || 0 GAIN_AG
  4 || 0 CheckBollingerSignal
  5 || 0 DetectaCruzamentoMACD
  6 || 0 || 9 Vel_Acc
  7 || 0 VerificarDirecaoEMA
  8 || 0 IchimokuSignal
*/
//+------------------------------------------------------------------+
// 1.1 proporcoes
//-------------------------------------------------------------------+
sinput string   Robo = "RTM14 - Ate 2 metodos";    
//"RMV07L - Ate 2 metodos"
sinput string Versão ="M14 1.1";                  // H2 GOLD 2 anos
input int  MainOP1                 = 6;           //Metodo (0 a 9)
input int  MainOP2                 = 9;           //Metodo (0 a 9)
input int MM_media_lenta = 17; //Media gatilho, lenta
input int MM_media_media = 14; //Media gatilho, intermediaria
input int MM_media_rapida = 4; //Media gatilho, rapida
input int Max_dist = 12; //Distancia max para previsão de cruzamento
input double MAX_LOTE = 4;               // Lote maximo para operar
input double SL_size    =  2000;       // Tamanho maximo do LOSS (USD)
input bool inverso             = false;       // Inverter as operações
input bool usainv             = false;       // Auto invert
input int LimLAT = 70; //Limite indicador lateralização
input double DIA_MAX_USD_GAIN = 0;  // Max USD DIA GAIN (Fecha dia)
input double DIA_MAX_USD_LOSS = 0; // Max USD DIA LOSS (Fecha dia)
input double CUR_MAX_USD_GAIN = 0;  // OP Max USD SALDO GAIN //(FECHA ROBO)
input double CUR_MAX_USD_LOSS = 0;  // OP Max USD SALDO LOSS //(FECHA ROBO)
input bool KILL_OP = false; //OPT Desistencias
input bool TRAILING_STOP = false; // Trailing stops
input bool TRAILING_FAST = true; // Fast trailing stops
input int PROPR_SDO = 2; //Proporcional ao saldo 0, 1, 2
input double FatProj = 2.5; //Fator projecao
input double OPR_MAX_Perc_GAIN = 80;  // Max perc OPER GAIN (Abre outra oper)
input double OPR_MAX_Perc_LOSS = 100;  // Max perc OPER LOSS (Fecha e abre oper inversa)
input double Prev_from = 10;  // Ant para vel/acc
input bool VA_confirm = false;  // VA confirmada pela media
input int TQC = 10; //Teste TQC
input double GL_fator = 0.8; //Fator G&L
double QCL_val_ant = 0;
double QCL_val_now = 0;
 bool MandaTelegram       = false;       // Usar Telegram
 string OutroAtivo = "="; // Outro ativo para tendencia
 bool Xtend = false; //Usar tendencia
 int TT_media_tend = 21; //Media para a tendencia
 ENUM_TIMEFRAMES TT_Period = PERIOD_W1; //Tempo grafico para a tendencia
...
//input
double SALDOINI          = 0;           // Saldo Disponivel
//input
...
//DESLIGADOS
//input
int TEND_qp                    = 5; // Quantidade de periodos para analise de tendencia
//input
...
int DirC = 5; //Quant candles em tendencia
...
datetime allowed_until = D'2026.06.06 00:00';
ORDENS MvO[10];
double MedP_ant = 0;
double MedP_cor = 0;
double MedG_cor = 0;
...
//i
   ArrayInitialize(M1pre, 0.0);
   Ajuste_AC = 0;
   Ajuste_Q = 0;
   Qsaldo = SALDO_Corrigido();
// LATERAL
   ...
   if(DIA_FIM == false)
     {
      ...
      if(CountSeconds(60, 5) == true)
        {
         int VA_op = Vel_Acc(Symbol(), Prev_from);
        }
     }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
   ...
   if((NewCandle==true)&&(X_ExpertRemove==false))
      ...
      */
      ...
      if(temOP == 0)
        {
         D_linHOR("H", highest_high, now, clrBlueViolet, 1);
         D_linHOR("L", lowest_low, now, clrBlueViolet, 1);
        }
      MOSTRA();
...
int Orders_ON()
  {
   ...
   if((KILL_OP == true) && (orders_count > 0))
     {
      Ve_se_DESISTE();
     }
...
void COMPRA_OU_VENDA()
  {
   ...
   NWOP = 0;
   string QUAL_OP = " ";
   if(orders_count < Q_OP_simult)
     {
      if(SALDO_Corrigido() > -SALDOINI)
        {
         if((MainOP1 == 1) || (MainOP1 == 0) || (MainOP1 == 9))
           {
            if(NWOP == 0)
              {
         ...
         if((MainOP1 == 5) || (MainOP1 == 0))
           {
            if(NWOP == 0)
              {
               NWOP = DetectaCruzamentoMACD(MM_media_rapida, MM_media_lenta, MM_media_media, PRICE_CLOSE);
               /*
                NWOP = CheckMACDCross(
                          _Symbol,
                          _Period,
         ...
         if((MainOP1 == 6) || (MainOP1 == 0) || (MainOP1 == 9))
           {
            if(NWOP == 0)
              {
               NWOP = Vel_Acc(Symbol(), Prev_from);
               if((NWOP > 0) && (VA_confirm == true))
                 {
                  int DMC = VerificarDirecaoEMA(Prev_from, Prev_from/2);
                  if(DMC != NWOP)
                    {
                     NWOP = 0;
                    }
         ...
         if((MainOP1 == 8) || (MainOP1 == 0))
           {
            if(NWOP == 0)
              {
               NWOP = IchimokuSignal(_Symbol, _Period, 9, 26, 52);
               if(NWOP > 0)
                 {
                  //Print("NWOP ",NWOP," ===");
                 }
         ...
         if((NWOP == 0) && (MainOP1 < MainOP2))
           {
            if((MainOP2 == 1) || (MainOP2 == 0) || (MainOP2 == 9))
              {
               if(NWOP == 0)
                 {
                  NWOP = CROSS_MEDIAS(_Symbol, MM_media_rapida, MM_media_lenta);
            ...
            if((MainOP2 == 6) || (MainOP2 == 0) || (MainOP2 == 9))
              {
               if(NWOP == 0)
                 {
                  NWOP = Vel_Acc(Symbol(), Prev_from);
         ...
         if((KILL_OP == true) && (CUR_OP > 0) && (CUR_OP != NWOP))
           {
            Close_Order();
           }
...
//   Print("Q ",qm++," Place_Order ", Oque," (", TENDENCIA_c_v,") Price ",requisicao.price," StopLoss ",requisicao.sl," StopGain ",requisicao.tp);
   ...
   bool dls = ObjectDelete(0, "LTen");
   double DifH = MathAbs(requisicao.tp - requisicao.price);
   double DifL = MathAbs(requisicao.sl - requisicao.price);
   double GLprop = DifH / DifL;
   DifH = MathAbs(requisicao.price - highest_high);
   DifL = MathAbs(requisicao.price - lowest_low);
   double HLprop = DifH / DifL;
//   Print ("GLP ", C_V, " ", GLprop, " ", HLprop);
   STR_SDO_INI_OPER = SALDO_Corrigido();
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
void DesenharReta(const string nome, const int subWindow,
                  ...
                  color cor, int width)
  {
// Cria o objeto de linha de tendencia (reta)
...
//+------------------------------------------------------------------+
//| Retorna a tendencia com base nos slopes                          |
//| N: número de candles a considerar                                |
//| limiar: valor mínimo para considerar uma tendencia               |
...
int DetectarTendencia(int N = 20, double limiar = 0.0001)
  {
   ...
   if(mediaSlope > limiar)
      ...
      if(mediaSlope < -limiar)
         return 2; // Tendencia de baixa
      else
         return 0; // Sem tendencia definida (neutra)
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
         ...
         if(m_position.Symbol() == _Symbol && m_position.Magic() == O_magic_number)
           {
            ...
            if((SIT_LIMITE == " ") && (TRAILING_STOP == true))
               if(TRAILING_FAST == true)
                 {
                  if(OP == 1)
                    {
                     VslPosition = SymbolInfoDouble(Symbol(),SYMBOL_BID)-(SPREAD*2);
                     TMPslPosition = NormalizeDouble(fmax(slPosition, VslPosition),2);
                     NEWslPosition = fmin(tpPosition, TMPslPosition);

                     NEWslPosition = TMPslPosition;

                    }
                  if(OP == 2)
                    {
                     VslPosition = SymbolInfoDouble(Symbol(),SYMBOL_ASK)+(SPREAD*2);
                     TMPslPosition = NormalizeDouble(fmin(slPosition, VslPosition),2);
                     NEWslPosition = fmax(tpPosition, TMPslPosition);
               ...
               else
                 {
                  ...
                  if((OP == 2) && (SymbolInfoDouble(Symbol(),SYMBOL_ASK) < VTS_valop-gap) && (slPosition > VTS_valop))
                    {
                     VslPosition = VTS_valop;
                     TMPslPosition = NormalizeDouble(fmin(TMPslPosition, VslPosition),2);
                    }
...
void Ve_se_DESISTE()
  {
   ...
   if(XFN > 1)
     {
      TendMenor =  Vel_Acc(FrameName[XFN-1], Prev_from);
     }
   int TendMaior = 0;
   if(XFN < 18)
     {
      TendMaior =  Vel_Acc(FrameName[XFN+1], Prev_from);
     }
   ulong posTicket;
   bool posSelected;
   V_CURRENT_ASK = SymbolInfoDouble(Symbol(),SYMBOL_ASK);
         ...
         if(m_position.Symbol() == _Symbol && m_position.Magic() == O_magic_number)
           {
            ...
            if((SIT_NOW == "L") && (TendMenor > 0) && (OP != TendMenor))
              {
               Oque = "DESISTE";
               Close_Order();
              }
...
// Cruzamento entre direção e tendencia
   ...
   if(OP == "S")
     {
      ...
      /*
            Print(
               ...
               " V.L ", TmpX[4],
               " S.I ",TmpX[5],
               " H.H ",NormalizeDouble(highest_high,0),
               " L.L ",NormalizeDouble(lowest_low,0),
               " D.G ",NormalizeDouble((highest_high-StringToDouble(TmpX[3])),0),
               " D.L ",NormalizeDouble((lowest_low-StringToDouble(TmpX[4])),0),
               " O.G ",MathAbs(NormalizeDouble((StringToDouble(TmpX[3])-StringToDouble(TmpX[2])),0)),
               " O.L ",MathAbs(NormalizeDouble((StringToDouble(TmpX[4])-StringToDouble(TmpX[2])),0))
            );
      */
     }
   if(OP == "V")
...
// Desenhar a linha de tendencia com base nos QTD_MD pontos
...
//   Print("Intensidade da Tendencia: ", intensidade, " ",direcao);
   DIR = intensidade;
   return DIR;
  }
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Detectar tendencia com LWMA e relatório de erro no CopyBuffer    |
//+------------------------------------------------------------------+
...
// Determina a direção da tendencia
...
void High_Low()
  {
   ...
   V_CURRENT_BID = SymbolInfoDouble(Symbol(),SYMBOL_BID);
   SPREAD = V_CURRENT_ASK - V_CURRENT_BID;
   highest_high = GL_fator * (iHigh(SYMB, tempoG, iHighest(SYMB, tempoG, MODE_HIGH, MathRound(Max_dist), 0)));
   lowest_low = GL_fator * (iLow(SYMB, tempoG, iLowest(SYMB, tempoG, MODE_LOW,MathRound(Max_dist), 0)));
   if(addSPREAD == true)
     {
      double fsp = 1.5;
...
double PrSdo(double valor)
  {
   ...
   if((PROPR_SDO == 2) && (SALDO_Corrigido() > STR_SDO_INI_ROBO))
     {
      valor = valor * (SALDO_Corrigido() / STR_SDO_INI_ROBO);
     }
   return valor;
...
//|                                                                  |
//+------------------------------------------------------------------+
int Vel_Acc(string mySymbol,int N)
  {
   int resp = 0;
   double M1close[60];
   double M1vel[60];
   double M1acc[60];
   ArrayInitialize(M1close, 0.0);
   ArrayInitialize(M1vel, 0.0);
   ArrayInitialize(M1acc, 0.0);
   VELpond=0;
   ACCpond=0;
   if(GetLastNCloses(mySymbol, N+1, M1close))
     {
      for(int i = 0; i < N; i++)
        {
         M1vel[i] = M1close[i] - M1close[i+1];
        }
      VELpond = WeightedAverageDesc(M1vel, N);
      for(int i = 0; i < N; i++)
        {
         M1acc[i] = M1vel[i] - M1vel[i+1];
        }
      ACCpond = WeightedAverageDesc(M1acc, N);
     }
   int Prev_to = MathAbs(N/2);
   double PDist = (VELpond * Prev_to) + (Prev_to * Prev_to * ACCpond * 0.5);
   if(M1pre[0] == 0)
     {
      M1pre[0] = M1close[0];
     }
   double previsto = M1pre[0];
   ArrayCopy(M1pre, M1pre, 0, 1, Prev_to - 1);
   double err_marg = M1close[0]/previsto;
   Ajuste_AC = Ajuste_AC + err_marg;
   Ajuste_Q++;
   err_marg = Ajuste_AC/Ajuste_Q;
   double Vaiser = (PDist+M1close[0])*err_marg;
   M1pre[Prev_to-1] = Vaiser;
   string n1 = " ";
   string n2 = " ";
   double c_p = (100*M1close[0]/previsto);
   double PERCProvavel = (100*Vaiser/M1close[0]);
   if(c_p != 0)
     {
      if(PERCProvavel > 100.1)
        {
         n1 ="Compra ";
         resp = 1;
        }
      if(PERCProvavel < 99.9)
        {
         n1 ="Venda ";
         resp = 2;
        }
   ...
   /*
      if(n1 != " ")
        {
         Print(
            " Prant ",DoubleToString(previsto,2),
            " CurP ",DoubleToString(M1close[0],2),
            " c/pa ",DoubleToString(c_p,2),
            " Prev em ", Prev_to,
            " ", DoubleToString(Vaiser,2),
            " c/pp ",DoubleToString(PERCProvavel,2),
            " AJac/q ",(Ajuste_AC/Ajuste_Q),
            " ",n1
         );
        }
   */
...
//|                                                                  |
//+------------------------------------------------------------------+
bool GetLastNCloses(string symbol, int N, double &closeArray[])
  {
   ArrayResize(closeArray, N);
   int copied = CopyClose(symbol, PERIOD_CURRENT, 0, N, closeArray);
   if(copied == N)
     {
      return true;
     }
...
//|                                                                  |
//+------------------------------------------------------------------+
double WeightedAverageDesc(double &M[], int N)
  {
   if(N <= 0)
      return 0.0;
   ...
   for(int i = 0; i < N; i++)
     {
      int weight = N - i;  // maior peso para índice menor
      weighted_total += M[i] * weight;
     }
   double result = weighted_total / weight_sum;
   return result;
  }
//+------------------------------------------------------------------+
//| Routine IchimokuSignal                                           |
//| Retorna: 1 = compra, 2 = venda, 0 = nada                         |
//+------------------------------------------------------------------+
int IchimokuSignal(string symbol,
                   ENUM_TIMEFRAMES timeframe,
                   int tenkan_sen = 9,
                   int kijun_sen  = 26,
                   int senkou_span_b = 52)
  {
// criar handle do indicador Ichimoku
   int handle = iIchimoku(symbol, timeframe, tenkan_sen, kijun_sen, senkou_span_b);
   if(handle == INVALID_HANDLE)
     {
      Print("Erro ao criar handle Ichimoku para ", symbol);
      return(0);
     }

// Definir arrays para copiar valores
...
// copiando valores: buffer 0 = Tenkan, 1 = Kijun, 2 = SenkouSpanA, 3 = SenkouSpanB
   if(CopyBuffer(handle, 0, 0, 2, tenkan) < 0 ||
      CopyBuffer(handle, 1, 0, 2, kijun)  < 0 ||
      CopyBuffer(handle, 2, 0, 2, spanA)  < 0 ||
      CopyBuffer(handle, 3, 0, 2, spanB)  < 0)
     {
      Print("Erro ao copiar buffers Ichimoku, symbol=", symbol);
...
// obter o preço de fechamento da barra mais recente (índice 0)
   double close0 = iClose(symbol, timeframe, 0);

// calcular a “nuvem” no índice 0 (use spanA[0], spanB[0])
   double topCloud = MathMax(spanA[0], spanB[0]);
   double bottomCloud = MathMin(spanA[0], spanB[0]);

// condição de compra
   if(tenkan[0] > kijun[0] && close0 > topCloud)
     {
      return(1);
     }

// condição de venda
   if(tenkan[0] < kijun[0] && close0 < bottomCloud)
     {
      return(2);
     }
...
int ContaQCL()
  {
   QCL_val_now = SALDO_Corrigido();
   if(QCL_val_ant > 0)
     {
      if((QCL_val_ant >= QCL_val_now) && (My_Orders_ON() == true))
        {
         QCL_q++;
         Print("QCL ",QCL_q);
        }
