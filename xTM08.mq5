...
sinput string Versão ="V 2";                  //
input double SALDO_DISPONIVEL          = 0;           // Saldo Disponivel para o robo
//double sugsl = NormalizeDouble((SALDO_DISPONIVEL*0.15),2);
input double SL_MAXIMO    =  0;    //Risco Maximo em cada LOSS (USD)
//input
...
//input
int dias = 30; //Renova saldo
//input
int LimLAT = 85; //Limite indicador lateralização
//input
int XPROP = 0; //Proporcional ao saldo 0, 1, 2//input
double F_MAX_LOTE = 10;          //Lote maximo para operar
//input
...
// INIT MvO
   ...
   double tsdo = SL_MAXIMO / SDO1;
   SL_SIZE = SL_MAXIMO;
   if(SL_SIZE == 0)
     {
      SL_SIZE = NormalizeDouble((SDO1 * 15 / 100),2);
     }
   if(SL_SIZE > SDO1)
     {
      SL_SIZE = SDO1;
     }
   MAX_LOTE = NormalizeDouble(F_MAX_LOTE,2);
...
" SL_SIZE ",SL_SIZE,
" MAX_LOTE ",MAX_LOTE,
" DIA_MAX_USD_LOSS ",DIA_MAX_USD_LOSS,
" CUR_MAX_USD_GAIN ",CUR_MAX_USD_GAIN,
" CUR_MAX_USD_LOSS ",CUR_MAX_USD_LOSS
);
*/
   MOSTRA();
// LATERAL
...
void OnTick()
  {
   ...
   if((dias > 0) && (Quant_Dias(dias, lastCheck)))
     {
      qpr ++;
      //      //T Print("QPR ",qpr);
