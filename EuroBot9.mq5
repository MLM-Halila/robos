...
int OnInit()
  {
   ...
   INV_oper = inverso;
   InitialEquity   = SALDO_Corrigido();
   HistoryMaxSize  = 2 * LookbackPeriods;
   ArrayResize(EquityHistory, 0);
...
void OnTick()
  {
   ...
   if(picos && (CountSeconds(SampleSeconds, 3) == true))
     {
...
//      Print("X Equity Curve → Ação sugerida: ", action);
      ...
      /*
            ...
            //   if(action == "INVERSA")  OpenOppositePosition();
            if(action == "PARAR")
              {
               Close_all_Orders(1);
               Close_all_Orders(2);
               Print("SALDO ",SALDO_Corrigido()," SSSSSSSSSSSSSSSSSS PARAR ");
               ExpertRemove();
              }
      */
     }
...
//+-----------------------------------------------------------------+
//| Retorna a ação sugerida                                         |
//| "FECHAR"     → feche todas as operações correntes               |
//| "MESMO"      → abra nova operação no mesmo sentido (adicionar)  |
//| "INVERSA"    → abra nova operação no sentido inverso (reverter) |
//| "PARAR"      → pare de operar (drawdown excessivo)              |
//| "HOLD"       → mantenha o que está aberto                       |
//+-----------------------------------------------------------------+
string AnalyzeEquityCurve()
  {
   int size = ArraySize(EquityHistory);
   if(size < LookbackPeriods + 1)
      return "HOLD"; // ainda não tem dados suficientes

   double current = EquityHistory[0];
   double rel_profit = (current - InitialEquity) / InitialEquity * 100.0; // F2

   bool winning = (current > InitialEquity);                              // F1

//--- Pico recente (F3 + F4)
   double max_recent = current;
   int    peak_idx   = 0;
   for(int i = 1; i < MathMin(LookbackPeriods, size); i++)
     {
      if(EquityHistory[i] > max_recent)
        {
         max_recent = EquityHistory[i];
         peak_idx   = i;
        }
     }
   bool   has_peak       = (peak_idx > 0) && (current < max_recent);
   double peak_intensity = has_peak ? (max_recent - current) / max_recent * 100.0 : 0.0; // F4

//--- Fundo recente (F3 + F4)
   double min_recent   = current;
   int    trough_idx   = 0;
   for(int i = 1; i < MathMin(LookbackPeriods, size); i++)
     {
      if(EquityHistory[i] < min_recent)
        {
         min_recent = EquityHistory[i];
         trough_idx = i;
        }
     }
   bool   has_trough       = (trough_idx > 0) && (current > min_recent);
   double trough_intensity = has_trough ? (current - min_recent) / min_recent * 100.0 : 0.0; // F4

//--- Velocidade média dos últimos 5 períodos (F5)
   int    vel_periods = MathMin(5, size - 1);
   double avg_speed   = 0.0;
   for(int i = 0; i < vel_periods; i++)
      avg_speed += (EquityHistory[i] - EquityHistory[i+1]) / InitialEquity * 100.0;
   if(vel_periods > 0)
      avg_speed /= vel_periods;

//--- Decisões baseadas em F1-F5
   if(rel_profit < -MaxDrawdownPercent)
      return "PARAR";

   if(has_peak && winning && peak_intensity > PeakDipPercent)
      return "FECHAR";                                 // pico passou + estamos em lucro → realize

   if(has_trough && !winning && trough_intensity > TroughRisePercent)
      return "INVERSA";                                // fundo em perda → reverter (espera recuperação)

   if(avg_speed > SpeedThresholdPercent && winning)
      return "MESMO";                                  // subindo rápido e em lucro → adicionar

   if(avg_speed < -SpeedThresholdPercent && !winning)
      return "FECHAR";                                 // caindo rápido em perda → corte
...
//|                                                                  |
//+------------------------------------------------------------------+
void AddEquityToHistory(double equity)
  {
   int size = ArraySize(EquityHistory);
   Print("XX SIZE ",size," ",QSX++);
   if(size < HistoryMaxSize)
      ArrayResize(EquityHistory, size + 1);

// desloca tudo uma posição para a direita (mantém [0] = mais novo)
