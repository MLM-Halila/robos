...
sinput string   Robo = "EuroBot8";
sinput string Versao ="1.0";                  //btc
input group " "
...
input int      Lookback_Candles = 50;      // Nº de candles históricos para análise
input int      Forecast_N       = 10;      // Nº de candles a projetar
...
// //
//-------------------------------------------------------------------
// Rotina principal: Projeta e detecta cruzamentos projetados
//-------------------------------------------------------------------
int ProjectAnalysis(const string symbol, ENUM_TIMEFRAMES timeframe)
  {
   int PREV = 0;
   double highArr[], lowArr[], bbUpArr[], bbLoArr[];

   if(!GetPriceArrays(symbol, timeframe, Lookback_Candles,
                      highArr, lowArr, bbUpArr, bbLoArr))
     {
      Print("Erro ao coletar dados históricos.");
      return 0;
     }

   int total = ArraySize(highArr);
   int start = total - Lookback_Candles;

   double histHigh[], histLow[], histBBUp[], histBBLo[];
   ArrayResize(histHigh, Lookback_Candles);
   ArrayResize(histLow, Lookback_Candles);
   ArrayResize(histBBUp, Lookback_Candles);
   ArrayResize(histBBLo, Lookback_Candles);

   for(int i=0; i<Lookback_Candles; i++)
     {
      histHigh[i]  = highArr[start + i];
      histLow[i]   = lowArr[start + i];
      histBBUp[i]  = bbUpArr[start + i];
      histBBLo[i]  = bbLoArr[start + i];
     }

   double forecastHigh[], forecastLow[], forecastBBUp[], forecastBBLo[];
   LinearForecast(histHigh, Lookback_Candles, Forecast_N, forecastHigh);
   LinearForecast(histLow,  Lookback_Candles, Forecast_N, forecastLow);
   LinearForecast(histBBUp,  Lookback_Candles, Forecast_N, forecastBBUp);
   LinearForecast(histBBLo,  Lookback_Candles, Forecast_N, forecastBBLo);
   bool crossUpper = false;
   bool crossLower = false;
   Print("=== Projecoes de Precos e Bollinger Bands (próx N candles) ===");
   for(int k=0; k<Forecast_N; k++)
     {
      //      bool crossUpper = (forecastHigh[k] > forecastBBUp[k]);
      //      bool crossLower = (forecastLow[k]  < forecastBBLo[k]);
      crossUpper = (forecastHigh[k] > forecastBBUp[k]);
      crossLower = (forecastLow[k] < forecastBBLo[k]);
      if(crossUpper || crossLower)
        {
         PrintFormat(
            "Candle %+d: HighProj=%.5f | LowProj=%.5f | BBUp=%.5f | BBLo=%.5f | CrossUp=%s | CrossLo=%s",
            (k+1),
            forecastHigh[k],
            forecastLow[k],
            forecastBBUp[k],
...
//-------------------------------------------------------------------
// Função para obter historico de máximos, mínimos, Bollinger upper/lower
//-------------------------------------------------------------------
bool GetPriceArrays(
   const string symbol,
  ...
  {
   int total = lookback + BB_Period + 10; // espaço extra para cobertura de histórico

   ArrayResize(highArr, total);
   ArrayResize(lowArr,  total);
   ArrayResize(bbUpperArr, total);
   ArrayResize(bbLowerArr, total);

// cópia de máximos e mínimos
   if(CopyHigh(symbol, timeframe, 0, total, highArr) < total)
      return false;
   if(CopyLow(symbol, timeframe, 0, total, lowArr)   < total)
      return false;
...
// Bollinger Bands
   ...
   if(CopyBuffer(handle, 1, 0, total, bbUpperArr) <= 0)
      return false;
   if(CopyBuffer(handle, 2, 0, total, bbLowerArr) <= 0)
      return false;
...
//-------------------------------------------------------------------
// Função de previsão linear (tendência) para projeção de dados
//-------------------------------------------------------------------
void LinearForecast(const double &data[], int nData, int nForecast, double &forecast[])
  {
   ...
   double denom = (nData * sumX2 - sumX * sumX);
   if(denom == 0.0)
     {
      ArrayResize(forecast, nForecast);
      for(int j=0; j<nForecast; j++)
         forecast[j] = data[nData-1];
      return;
     }

   double b = (nData * sumXY - sumX * sumY) / denom;
   double a = (sumY - b * sumX) / nData;

   ArrayResize(forecast, nForecast);
   for(int j=0; j<nForecast; j++)
     {
      int idx = nData + j; // posição além do último histórico
      forecast[j] = a + b * idx;
     }
  }
//DESISTENCIA---------------------------------------------------+
